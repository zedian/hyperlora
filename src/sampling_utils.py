## HYPERFORMER CODE https://github.com/rabeehk/hyperformer
"""Implements a distributed sampler to sample different dialects with
temperature sampling in a way to make sure that the same dialect is
...
class MultiDialectBatchSampler(Sampler[T_co]):
    ...
    def __init__(self, dataset_sizes: List[int], batch_size: int, temperature: float,
        ...
        """Constructor for MultiDialectBatchSampler.
        Args:
    ...
    def generate_dialect_distribution(self):
    ...
    def __iter__(self):
        ...
        # model is based on dialects, we need to make sure the same dialect is selected
        # across different processes.
        dialect_distribution: torch.Tensor = self.generate_dialect_distribution()

        # Chooses the dialects which will be used in each batch in one epoch.
        # With passing generator, we make sure this choice is consistent across
        # different processes.
        batch_dialect_assignments = torch.multinomial(dialect_distribution,
                                                   self.num_batches_per_epoch, replacement=True, generator=generator)

        for batch_dialect in batch_dialect_assignments:
            # Gets the number of samples of the selected datasets available for the
            # current rank.
            num_dialect_samples = self.rank_dataset_sizes[batch_dialect]
            # Computes the random samples from the chosen dataset.
            indices = torch.randint(low=0, high=num_dialect_samples, size=(self.batch_size,), generator=generator).tolist()
            # Converts the selected indices to the global indices on the given dataset.
            results = (self.dataset_offsets[batch_dialect] + torch.tensor(self.rank_indices[batch_dialect])[indices]).tolist()
            yield results
